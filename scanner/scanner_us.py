...
from tickers_us import get_all_tickers

SCANNER_DIR = Path(__file__).resolve().parent
ROOT = SCANNER_DIR.parent
SITE_DIR = ROOT / "site"
HISTORY_DIR = SITE_DIR / "history_us"
SITE_DIR.mkdir(exist_ok=True)
HISTORY_DIR.mkdir(exist_ok=True)

# Universe cache：保存哪些代碼有實質數據（避免每次重新探索）
UNIVERSE_FILE = SCANNER_DIR / "universe_cache_us.json"

HKT = timezone(timedelta(hours=8))

# 篩選參數
MIN_TURNOVER_M = 10.0         # 平均成交額 1000萬美元 (~7800萬 HKD)
MIN_TURNOVER = MIN_TURNOVER_M * 1_000_000  # USD
...
def fetch_data(tickers: list[str], period: str = "1y", explore: bool = False) -> dict:
    ...
    if explore:
        # Explore mode：rebuild universe (refresh from sources)
        from tickers_us import build_universe
        print("探索模式：rebuild universe...")
        build_universe()
        # 重新 load tickers
        from importlib import reload
        import tickers_us
        reload(tickers_us)
        target = tickers_us.get_all_tickers()
        print(f"探索模式: {len(target)} 隻")
    else:
        # Fast mode：直接用 tickers (來自 universe_us.json, 已 builder 好)
        # 唔再依賴 universe_cache_us.json，避免被 partial cache 限制
        target = list(tickers)
        print(f"快速模式: {len(target)} 隻 (universe_us.json)")
...
METADATA_FILE = SCANNER_DIR / "metadata_us.json"
MIN_MARKET_CAP = 2_000_000_000  # 20 億美元 (~155 億 HKD)
...
def update_metadata(tickers: list, force_refresh: bool = False) -> dict:
    ...
    for ticker in tickers:
        ...
        if info and (info.get("name_en") or info.get("market_cap", 0) > 0):
            # 美股版：直接用英文名做主名稱
            # 如果有自訂中文/簡稱 cache (names_us.json) 就 prefer 嗰個
            custom = chinese_names.get(code, "")
            info["name"] = custom if custom else info.get("name_en", code)
            meta[ticker] = info
            fetched += 1
            if fetched % 10 == 0:
...
STREAKS_FILE = SITE_DIR / "streaks_us.json"
...
def load_company_names() -> dict:
    names_file = SCANNER_DIR / "names_us.json"
...
def run_scan(explore: bool = False) -> dict:
    ...
    print(f"\n{'=' * 60}")
    print(f"美股強勢股 Scanner v1 - {today}")
    print(f"{'=' * 60}")
    ...
    MIN_COVERAGE_RATIO = 0.30  # 至少 30% universe 拿到數據
    ...
    # 美股版：暫時用 code 做預設，等 metadata fetch 之後會被覆寫成英文名
    chinese_names = load_company_names()  # 美股版可能為空
    for r in results:
    ...
    output = {
        ...
        "total_universe": len(get_all_tickers()),
        "data_available": len(data),
    ...
    # 寫入結果
    with open(SITE_DIR / "data_us.json", "w") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    with open(SITE_DIR / "data_us.js", "w") as f:
        f.write("window.SCANNER_DATA_US = ")
        json.dump(output, f, ensure_ascii=False, indent=2)
        f.write(";\n")
